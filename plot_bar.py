    ...
    for workload_type in df['workload_type'].unique():
        for op_type in df['op_type'].unique():
            for coro in sorted(df['coros'].unique()):
                ...
                # 标题美化
                op_map = {'read': 'Read', 'update': 'Update', 'insert': 'Load'}
                title = f"{workload_type.capitalize()} {op_map.get(op_type.strip('_'), op_type.capitalize())}"
                plt.title('Load')
                plt.legend()
                plt.tight_layout()
                fname = f"output/barplots/LoadIOPS_{load_num}_{workload_type}_{op_type}_coro{coro}.png"
                plt.savefig(fname)
                plt.close()
                ...
                plt.legend()
                plt.tight_layout()
                fname = f"output/barplots/RunIOPS_{load_num}_{workload_type}_{op_type}_coro{coro}.png"
                plt.savefig(fname)
                plt.close()